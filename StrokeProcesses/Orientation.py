...
class Orientation:
	...
	def diffusion(self):
		length, width,_ = self.image.shape
		f = Ip.direction_sketch(self.image)                       # f is the sketch line graph whose format is f[(i, j)]: [0/1, angle]
		...
		dic, eff = Ip.iteration(self.iteration_number, dic, eff, length, width)
		self.dic=dic
		self.eff=eff
